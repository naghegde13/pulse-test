# EBCDIC DISCOVERY AGENT — OPERATING PROTOCOL v2
...
START
  ...
  │   │   └─ NO → Continue to Q2.
  │   │
  │   ├─ Q2: Only 1 row from a multi-KB file?
  │   │   │
  │   │   ├─ YES → RDW endianness is wrong (one huge record swallowed the file).
  │   │   │   Toggle is_rdw_big_endian. Keep everything else the same (F-15).
  │   │   │
  │   │   └─ NO → Continue to Q3.
  │   │
  ...
  │   │   │   as Scala object, not applied. This is a known backend issue. Keep the correct map in
  │   │   │   your config; it may take effect on the next run. Focus on other issues in the meantime.
  │   │   │
...
If a config that previously produced rows suddenly produces 0 rows after a copybook rewrite, the rewrite is the problem, not the framing. Immediately revert the copybook to the version that was active when rows were last produced. Check the Cobrix-derived width in every run result and compare to previous runs.
...
4. **Segment discrimination** is working (derivedSegmentField is populated, not null).
