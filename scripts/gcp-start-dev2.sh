...
if gcloud sql instances describe "${SQL_INSTANCE}" --project="${PROJECT_ID}" &>/dev/null; then
  ...
  echo "[DB] Instance not found. Run 'scripts/gcp-deploy-dev2.sh' first."
  exit 1
fi

echo ""
echo "=== Started (DEV2) ==="
FRONTEND_URL=$(gcloud run services describe pulse-dev2-frontend --region="${REGION}" --format="value(status.url)" 2>/dev/null || echo "N/A")
BACKEND_URL=$(gcloud run services describe pulse-dev2-backend --region="${REGION}" --format="value(status.url)" 2>/dev/null || echo "N/A")
