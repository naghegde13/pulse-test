...
########################################################################
# PULSE GCP Cloud Run Deploy Script — DEV2
# Deploys: Cloud SQL (Postgres), Backend (Spring Boot), Frontend (Next.js)
########################################################################

PROJECT_ID="gcp-mat"
REGION="us-central1"
REPO="pulse-dev2-repo"
REGISTRY="${REGION}-docker.pkg.dev/${PROJECT_ID}/${REPO}"
SQL_INSTANCE="pulse-dev2-db"
FRONTEND_SERVICE="pulse-dev2-frontend"
BACKEND_SERVICE="pulse-dev2-backend"
VPC_NETWORK="mat-iap-default"
VPC_CONNECTOR="pulse-dev2-connector"

# Parse flags
COMPONENT="${1:-all}"  # all | frontend | backend | db

echo "=== PULSE GCP Deploy (DEV2) ==="
...
deploy_db() {
  ...
  if gcloud sql instances describe "${SQL_INSTANCE}" --project="${PROJECT_ID}" &>/dev/null; then
    ...
    echo "[DB] Creating Cloud SQL instance (private IP only)..."
...
########################################################################
deploy_frontend() {
  echo "--- [Frontend] Building image (BUILD_ENV=dev2) ---"
  docker build --platform linux/amd64 \
    --build-arg BUILD_ENV=dev2 \
